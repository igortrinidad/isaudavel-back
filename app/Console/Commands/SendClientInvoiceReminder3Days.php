...
class SendClientInvoiceReminder3Days extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'invoice:client-reminder-3-days';
     ...
     * @var string
     */
    protected $description = 'Send a reminder of open invoices 3 days before expiration to clients';
    ...
    public function handle()
    {
        ...
        $expire_at = Carbon::now()->addDays(3)->format('Y-m-d');
        ...
        foreach ($invoices as $invoice){
            ...
            $data['messageOne'] = 'Estamos passando apenas para lembrá-lo(a) que sua fatura da empresa '. $invoice->company->name .', referente ao plano '.$invoice->subscription->plan->name.' com valor de R$'. number_format($invoice->value,2,',', '.'). ' vence em '. $invoice->expire_at.'.';
